				...
				<tbody>
					@foreach($hangsanxuat as $value)
						<tr>
							...
							<td class="text-center"><img src="{{ env('APP_URL') . '/storage/app/' . $value->hinhanh . '?t=' . time() }}" width="100" class="img-thumbnail" /></td>
							<td>{{ $value->tenhang }}</td>
							...
							<td class="text-center">
								<a href="{{ route('admin.hangsanxuat.xoa', ['id' => $value->id]) }}" onclick="return confirm('Bạn có muốn xóa bài viết {{ $value->tenhang }} không?')">
